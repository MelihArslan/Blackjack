...
import nl.meliharslan.ewa.database.domein.Table;
import nl.meliharslan.ewa.database.domein.dto.PlayerDTO;
import nl.meliharslan.ewa.database.exception.ApiRequestException;
import nl.meliharslan.ewa.database.repo.PlayerRepository;
import nl.meliharslan.ewa.database.service.base.HandService;
import nl.meliharslan.ewa.database.service.base.PlayerService;
import nl.meliharslan.ewa.database.utilities.CustomModelMapper;
...
public class PlayerServiceImpl implements PlayerService {
    ...
    public Player executeAction(String action, int playerId, int handId) {
        switch (action) {
            case "hit":
                return hit(playerId, handId);
            case "double":
                return doubleDown(playerId, handId);
            case "split":
                return split(playerId, handId);
            case "stand":
                return stand(playerId, handId);
            case "surrender":
                return surrender(playerId, handId);
            default:
                throw new ApiRequestException(String.format(ACTION_NOT_ALLOWED, action));
        }
    }
}
